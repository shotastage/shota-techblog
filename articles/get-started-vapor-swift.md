...
Server-sideの使用言語は、半ばフレームワークありきで選択されることが多いです。例えば、Ruby on RailsやJava Springは王道ですし最近ですとPythonのDjangoなども人気です。
...
地獄が悩ましいポイントでした。SwiftのOptional型がないので、nilチェックが漏れていてもコンパイルは通るので実行してから`panic error`に遭遇することになります。
k8sでコンテナの再起動をかけるのがGoにおいてのベストプラクティスかもしれませんが、**MagicApp**ではインフラを過剰に高度
...
async fn handler(req: Request) -> Response {
    let user = req.user();            // req から参照を取る（&User）
    let body = req.body_json();        // ここで req をまた使いたい
    // ^ 「req は user に借用されてるから使えない」系で詰まりがち
    ...
}
```
